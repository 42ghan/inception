...
if [[ ! -d /var/lib/mysql/mysql ]]; then
	...
	&& mariadb -u root -e "CREATE USER IF NOT EXISTS 'wp'@'$WP_REMOTE_ADDRESS';" \
	&& mariadb -u root -e "GRANT ALL PRIVILEGES ON wordpress.* TO 'wp'@'$WP_REMOTE_ADDRESS' IDENTIFIED BY '$DB_PASSWD';" \
	&& mariadb -u root -e "CREATE USER IF NOT EXISTS 'wp'@'$ADMINER_REMOTE_ADDRESS';" \
	&& mariadb -u root -e "GRANT ALL PRIVILEGES ON wordpress.* TO 'wp'@'$ADMINER_REMOTE_ADDRESS' IDENTIFIED BY '$DB_PASSWD';"
